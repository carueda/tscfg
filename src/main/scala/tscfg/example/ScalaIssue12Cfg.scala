...
object ScalaIssue12Cfg {
  object Boolean {
    def apply(c: scala.Option[com.typesafe.config.Config]): Boolean = {
      Boolean(
  ...
  object Option {
    def apply(c: scala.Option[com.typesafe.config.Config]): Option = {
      Option(
  ...
  object String {
    def apply(c: scala.Option[com.typesafe.config.Config]): String = {
      String(
        c.map(c => if(c.hasPathOrNull("bar")) c.getString("bar") else "baz").get
      )
    }
  ...
  object Int {
    def apply(c: scala.Option[com.typesafe.config.Config]): Int = {
      Int(
  ...
  def apply(c: scala.Option[com.typesafe.config.Config]): ScalaIssue12Cfg = {
    ScalaIssue12Cfg(
      Boolean(c.map(c => if (c.hasPath("Boolean")) Some(c.getConfig("Boolean")) else None).get),
      Option(c.map(c => if (c.hasPath("Option")) Some(c.getConfig("Option")) else None).get),
      String(c.map(c => if (c.hasPath("String")) Some(c.getConfig("String")) else None).get),
      Int(c.map(c => if (c.hasPath("int")) Some(c.getConfig("int")) else None).get)
    )
  }
}
