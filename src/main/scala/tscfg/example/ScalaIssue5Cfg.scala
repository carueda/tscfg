...
object ScalaIssue5Cfg {
  object Foo {
    object Config {
      ...
      def build(c: scala.Option[com.typesafe.config.Config]): Config = {
        Config(
    ...
    def build(c: scala.Option[com.typesafe.config.Config]): Foo = {
      Foo(
        Config.build(c.map(c => if (c.hasPath("config")) Some(c.getConfig("config")) else None).get)
      )
    }
  }
  case class Foo(
  ...
  def apply(c: com.typesafe.config.Config) = build(Some(c))
  def apply() = build(None)
  def build(c: scala.Option[com.typesafe.config.Config]): ScalaIssue5Cfg = {
    ScalaIssue5Cfg(
