...
object ScalaIssue41Cfg {
  ...
  object C {
    ...
    def apply(
      ...
      ScalaIssue41Cfg.C(
        ...
        f =
          if (c.hasPathOrNull("f"))
            scala.Some(ScalaIssue41Cfg.C.F(c.getConfig("f"), parentPath + "f.", $tsCfgValidator))
          else None
      )
    }
  }
  ...
  def apply(c: com.typesafe.config.Config): ScalaIssue41Cfg = {
    ...
    val $result = ScalaIssue41Cfg(
      ...
      i =
        if (c.hasPathOrNull("i")) scala.Some($_L$_dbl(c.getList("i"), parentPath, $tsCfgValidator))
        else None
    )
