...
object ScalaIssue11Cfg {
  ...
  object Foo {
    def apply(c: com.typesafe.config.Config, parentPath: java.lang.String, $tsCfgValidator: $TsCfgValidator): ScalaIssue11Cfg.Foo = {
      ScalaIssue11Cfg.Foo(
  ...
  def apply(c: com.typesafe.config.Config): ScalaIssue11Cfg = {
    ...
    val $result = ScalaIssue11Cfg(
      foo = ScalaIssue11Cfg.Foo(if(c.hasPathOrNull("foo")) c.getConfig("foo") else com.typesafe.config.ConfigFactory.parseString("foo{}"), parentPath + "foo.", $tsCfgValidator)
    )
