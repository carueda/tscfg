...
object ScalaExample4tplCfg {
  ...
  object Endpoint {
    ...
    object Notifications {
      ...
      def apply(
        ...
        ScalaExample4tplCfg.Endpoint.Notifications(
          emails = $_LScalaExample4tplCfg_Endpoint_Notifications_Emails$Elm(
            c.getList("emails"),
            parentPath,
            $tsCfgValidator
          )
      ...
      private def $_LScalaExample4tplCfg_Endpoint_Notifications_Emails$Elm(
          cl: com.typesafe.config.ConfigList,
          parentPath: java.lang.String,
          $tsCfgValidator: $TsCfgValidator
      ): scala.List[ScalaExample4tplCfg.Endpoint.Notifications.Emails$Elm] = {
    ...
    def apply(
      ...
      ScalaExample4tplCfg.Endpoint(
        notifications = ScalaExample4tplCfg.Endpoint.Notifications(
          if (c.hasPathOrNull("notifications")) c.getConfig("notifications")
          else com.typesafe.config.ConfigFactory.parseString("notifications{}"),
        ...
        stuff =
          if (c.hasPathOrNull("stuff"))
            scala.Some(
              ScalaExample4tplCfg.Endpoint
                .Stuff(c.getConfig("stuff"), parentPath + "stuff.", $tsCfgValidator)
            )
          else None
      )
    }
  ...
  private def $_L$_L$_dbl(
      cl: com.typesafe.config.ConfigList,
      parentPath: java.lang.String,
      $tsCfgValidator: $TsCfgValidator
  ): scala.List[scala.List[scala.Double]] = {
    import scala.jdk.CollectionConverters._
    cl.asScala
      .map(cv =>
        $_L$_dbl(cv.asInstanceOf[com.typesafe.config.ConfigList], parentPath, $tsCfgValidator)
      )
      .toList
  }
  private def $_L$_dbl(
