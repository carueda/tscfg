...
object ScalaIssue12Cfg {
  ...
  def apply(c: com.typesafe.config.Config): ScalaIssue12Cfg = {
    ...
    val $result = ScalaIssue12Cfg(
      Boolean = ScalaIssue12Cfg.Boolean(
        if (c.hasPathOrNull("Boolean")) c.getConfig("Boolean")
        else com.typesafe.config.ConfigFactory.parseString("Boolean{}"),
        parentPath + "Boolean.",
        $tsCfgValidator
      ),
      Option = ScalaIssue12Cfg.Option(
        if (c.hasPathOrNull("Option")) c.getConfig("Option")
        else com.typesafe.config.ConfigFactory.parseString("Option{}"),
        parentPath + "Option.",
        $tsCfgValidator
      ),
      String = ScalaIssue12Cfg.String(
        if (c.hasPathOrNull("String")) c.getConfig("String")
        else com.typesafe.config.ConfigFactory.parseString("String{}"),
        parentPath + "String.",
        $tsCfgValidator
      ),
      int = ScalaIssue12Cfg.Int(
        if (c.hasPathOrNull("int")) c.getConfig("int")
        else com.typesafe.config.ConfigFactory.parseString("int{}"),
        parentPath + "int.",
        $tsCfgValidator
      )
    )
