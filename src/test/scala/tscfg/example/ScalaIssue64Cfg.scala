  ...
  object Test {
    def apply(c: com.typesafe.config.Config, parentPath: java.lang.String, $tsCfgValidator: $TsCfgValidator): ScalaIssue64Cfg.Test = {
      ScalaIssue64Cfg.Test(
        loadModelConfig = LoadModelConfig(if(c.hasPathOrNull("loadModelConfig")) c.getConfig("loadModelConfig") else com.typesafe.config.ConfigFactory.parseString("loadModelConfig{}"), parentPath + "loadModelConfig.", $tsCfgValidator)
      )
    }
  }
