...
object ScalaIssue64Cfg {
  ...
  object Test {
    def apply(c: com.typesafe.config.Config, parentPath: java.lang.String, $tsCfgValidator: $TsCfgValidator): ScalaIssue64Cfg.Test = {
      ScalaIssue64Cfg.Test(
        loadModelConfig = ScalaIssue64Cfg.LoadModelConfig(if(c.hasPathOrNull("loadModelConfig")) c.getConfig("loadModelConfig") else com.typesafe.config.ConfigFactory.parseString("loadModelConfig{}"), parentPath + "loadModelConfig.", $tsCfgValidator)
      )
    }
  }
  ...
  def apply(c: com.typesafe.config.Config): ScalaIssue64Cfg = {
    ...
    val $result = ScalaIssue64Cfg(
      test            = ScalaIssue64Cfg.Test(if(c.hasPathOrNull("test")) c.getConfig("test") else com.typesafe.config.ConfigFactory.parseString("test{}"), parentPath + "test.", $tsCfgValidator)
    )
