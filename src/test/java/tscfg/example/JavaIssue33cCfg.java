...
public class JavaIssue33cCfg {
  ...
  public static class Endpoint {
    ...
    public Endpoint(
        ...
        $TsCfgValidator $tsCfgValidator) {
      this.optObj =
          c.hasPathOrNull("optObj")
              ? new Endpoint.OptObj(c.getConfig("optObj"), parentPath + "optObj.", $tsCfgValidator)
              : new Endpoint.OptObj(
                  com.typesafe.config.ConfigFactory.parseString("optObj{}"),
                  parentPath + "optObj.",
                  $tsCfgValidator);
  ...
  public JavaIssue33cCfg(com.typesafe.config.Config c) {
    ...
    this.endpoint =
        c.hasPathOrNull("endpoint")
            ? new JavaIssue33cCfg.Endpoint(c.getConfig("endpoint"), "endpoint.", $tsCfgValidator)
            : new JavaIssue33cCfg.Endpoint(
